...
class RestauranteUseCaseTest {
    ...
    void testSaveRestaurante_Exito() {
        ...
        Rol rolPropietario = new Rol(ROL_PROPIETARIO, "PROPIETARIO");
        ...
        administrador.setRol(new Rol(ROL_ADMIN,"ADMINISTRADOR"));
    ...
    void testSaveRestaurante_UsuarioNoEsPropietario_LanzaExcepcion() {
        ...
        Rol rolAdmin = new Rol(ROL_EMPLEADO, "EMPLEADO");
    ...
    void testSaveRestaurante_UsuarioNoEsAdministrador_LanzaExcepcion() {
        ...
        Usuario administrador = new Usuario();
        administrador.setId(usuarioSesion);
        administrador.setRol(new Rol(ROL_PROPIETARIO,"EMPLEADO"));
    ...
    void testListarRestaurantesConPaginacionDevuelveSoloNombreYLogo() {
        ...
        when(restaurantePersistencePort.listarRestaurantes(pagina, tamanio)).thenReturn(listaEsperada);

        // Act
        List<Restaurante> resultado = restauranteUseCase.listarRestaurantes(pagina, tamanio);

        // Assert
        assertEquals(2, resultado.size());
        assertEquals("Arepa House", resultado.get(0).getNombre());
        assertEquals("http://abc.com", resultado.get(0).getUrlLogo());
        assertEquals("Burger Plaza", resultado.get(1).getNombre());
        assertEquals("http://xyz.com", resultado.get(1).getUrlLogo());

        verify(restaurantePersistencePort).listarRestaurantes(pagina, tamanio);
    }
